...
def davenport(color, input='r-J', output='r-i'):
	data = astropy.io.ascii.read(settings.prefix + 'inputs/davenport_table1.txt', names=['g-i','#','u-g','eu-g','g-r','eg-r','r-i','er-i','i-z','ei-z','z-J','ez-J','J-H','eJ-H','H-K','eH-K'])
	if input == 'r-J':
...
def pickles(color, input='R-J'):
	...
	coeff, stats = np.polynomial.polynomial.polyfit(x, y, 4, full=True)
	interpolator = np.polynomial.polynomial.Polynomial(coeff)
