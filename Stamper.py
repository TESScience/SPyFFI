...
class Stamper(Talker):
  ...
  def write(self):
    ...
    # write it out
    f = self.ccd.directory + 'postagestamptargets_{pos}_{name}.txt'.format(pos=self.ccd.pos_string, name=self.ccd.name)

    t.write(f, format='ascii.fixed_width', delimiter='|', bookend=False)
    self.speak('wrote stamp definition catalog to {}'.format(f))

  def fromNothing(self):
    self.speak('no postage stamps defined')
    self.ccd.stampimage = self.ccd.ones()

  def fromRandom(self, nstamps, radius=10):
    self.speak('randomly populating {} with {} postage stamps'.format(self.ccd.name, nstamps))
  ...
  def fromFile(self, filename):
    self.speak('loading RA and Dec stamp centers from {}'.format(filename))
  ...
  def populateStampImage(self):
    ...
    for i in range(len(x)):
      if (x[i]>=0)*(x[i]<self.ccd.xsize)*(y[i]>=0)*(y[i]<self.ccd.ysize):
          thisx = np.minimum(np.maximum(np.round(x[i]).astype(np.int) + xgrid, 0), self.ccd.xsize-1).astype(np.int)
          thisy = np.minimum(np.maximum(np.round(y[i]).astype(np.int) + ygrid, 0), self.ccd.ysize-1).astype(np.int)
          aperture = r <= radii[i]
